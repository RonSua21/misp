...
export default async function ApplicationDetailPage({
  ...
  // Guaranteed columns — never fails on missing migration columns
  const { data: app } = await db
    .from("applications")
    .select("id, referenceNumber, status, purpose, amountApproved, remarks, benefitProgramId, createdAt, updatedAt")
    .eq("id", id)
  ...
  // Phase-1 migration columns (snake_case) — fetch separately so a missing
  // column never causes the page to 404.
  const { data: appExtra } = await db
    .from("applications")
    .select("approval_level, rejection_code")
    .eq("id", id)
    .maybeSingle();
  ...
  const approvalLevel = ((appExtra as any)?.approval_level as number) ?? 0;
  const activeStep    = getActiveStep(approvalLevel, app.status as ApplicationStatus);

  const rejCode = ((appExtra as any)?.rejection_code as string | null) ?? null;
