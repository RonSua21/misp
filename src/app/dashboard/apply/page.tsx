...
export default async function ApplyPage() {
  ...
  if (profile.role === "GUEST") {
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Apply for Assistance</h1>
        </div>
        <div className="card p-8 text-center space-y-4">
          <div className="w-16 h-16 rounded-full bg-red-400/20 border border-red-400/30 flex items-center justify-center mx-auto">
            <UserX className="w-8 h-8 text-red-400" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-white">Restricted — Non-Makati Resident</h2>
            <p className="text-white/60 text-sm mt-2 max-w-sm mx-auto">
              MSWD benefit programs are exclusively for <strong className="text-white">Makati City residents</strong>.
              Your GPS verification determined that you are outside Makati City.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
            <Link href="/guest" className="btn-primary text-sm">
              <ExternalLink className="w-4 h-4" /> View Guest Services
            </Link>
            <Link href="/dashboard/profile#residency" className="btn-secondary text-sm">
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-white">Apply for Assistance</h1>
        <p className="text-white/60 text-sm mt-1">Complete all required fields and upload supporting documents.</p>
      </div>
      ...
      {!profile.residencyVerified && (
        <div className="flex items-start gap-3 p-4 bg-amber-400/10 border border-amber-400/25 rounded-xl text-sm text-amber-300">
          ...
          <div className="flex-1">
            <p className="font-semibold text-amber-200">Residency Verification Required</p>
            <p className="mt-0.5 text-amber-300/80">Verify your Makati City address before submitting.</p>
          </div>
          <Link href="/dashboard/profile#residency" className="shrink-0 flex items-center gap-1 font-semibold text-amber-200 hover:underline">
